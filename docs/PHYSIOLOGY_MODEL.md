...
`npm run audit:brain` regenerates [the complete source-candidate inventory](brain-vessel-coverage.json). In the packaged atlas, the defined spatial / naming rule yields **151 vascular candidates**: 79 exact name associations to schematic nodes, 60 grouped territory associations, 10 unresolved, and 2 adjacent orbital vessels. Every candidate has a unique source ID, classification, association basis, and laterality evidence. The 3D viewer can display this entire candidate set together. Coverage filters and individual source links are available below the brain map.

The 10 unresolved source candidates are paired anterior inferior cerebellar, anterior spinal, central-sulcus, and posterior temporal branch meshes. Several unsided names do not establish laterality through concept membership. The audit preserves that uncertainty; it does not infer verified vascular edges from a mesh bounding box. No venous meshes meet the rule, so the intracranial venous drawing remains a schematic without matching source geometry. These are concrete remaining source-data / topology gaps, not a claim of full vessel connectivity.
